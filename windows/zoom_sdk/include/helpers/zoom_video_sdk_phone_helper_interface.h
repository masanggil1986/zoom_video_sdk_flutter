 ...
 * @brief Phone welcome message multilingual information interface.
 */
class IZoomVideoSDKPhoneWelcomeMessageInfo
{
public:
	...
	/**
	 * @brief Get the current information's language code.
	 * @return The language code, such as "en-US", "zh-CN".
	 */
	virtual const zchar_t* getLanguageCode() = 0;

	/**
	 * @brief Get the current information's language name.
	 * @return The language name, such as "English (US)", "Chinese (China)".
	 */
	virtual const zchar_t* getLanguageName() = 0;

	/**
	 * @brief Get the current information's variant ID.
 ...
 * @note languageCode and variantId must be used together (both provided or both nullptr).
 *       Use getPhoneWelcomeMessageInfoList() to retrieve the list of valid language codes and variant IDs.
 *       If provided with an invalid value, invitePhoneUser() returns ZoomVideoSDKErrors_Invalid_Parameter.
 */
...
public:
	 ...
	 * @return true if join by phone is supported. Otherwise, false.
	 */
	...
	/**
	 * @brief Get the list of multilingual welcome message information.
	 * @return List of multilingual welcome message information.
	 *         Returns an empty list if the account doesn't support phone, or if no languages are configured.
	 * @note Use this method to query valid language codes and variant IDs before passing them to invitePhoneUser() via InvitePhoneUserInfo.
